...
import {
  ...
  useSignMessage,
  useSwitchChain
} from "./chunk-T5SNN5YK.js";
import {
  import_index
} from "./chunk-YOLEXXBR.js";
...
import {
  ...
  validate2 as validate4,
  withRetry,
  withTimeout
} from "./chunk-3MTZW5VS.js";
import {
  Field,
  FpInvertBatch,
  FpLegendre,
...
import {
  ...
  notImplemented,
  numberToBytesBE
} from "./chunk-47ZRFJWX.js";
import {
  require_react_dom
} from "./chunk-J4NESBNT.js";
import {
  useQuery
} from "./chunk-S25WRBQX.js";
import {
  require_jsx_runtime
...
import {
  mainnet
} from "./chunk-KK6HBESK.js";
import "./chunk-6B6UKE4X.js";
import {
  ...
  isAddress,
  numberToHex
} from "./chunk-FQ2YWGEQ.js";
import {
  formatAbi,
  formatAbiItem,
  parseAbi
} from "./chunk-AVNE4FUI.js";
import "./chunk-LILECVDN.js";
import {
  SHA256_IV,
  sha256,
  sha384,
    ...
    var _loop = function _loop2() {
      ...
      try {
        ...
        if (true) {
          ...
          var format6 = (v) => typeof v === "string" ? '"'.concat(v, '"') : v;
          var invalidPropValue = (prop2, value2, possibleValues) => {
            throw new SprinklesError('"'.concat(prop2, '" has no value ').concat(format6(value2), ". Possible values are ").concat(Object.keys(possibleValues).map(format6).join(", ")));
          };
          if (!sprinkle2) {
          ...
          if (typeof propValue === "string" || typeof propValue === "number") {
            ...
            if (!sprinkle2.values[propValue].defaultClass) {
              throw new SprinklesError('"'.concat(prop, '" has no default condition. You must specify which conditions to target explicitly. Possible options are ').concat(Object.keys(sprinkle2.values[propValue].conditions).map(format6).join(", ")));
            }
          }
              ...
              for (var _conditionName2 in propValue) {
                ...
                if (_value2 != null) {
                  ...
                  if (!sprinkle2.values[_value2].conditions[_conditionName2]) {
                    throw new SprinklesError('"'.concat(prop, '" has no condition named ').concat(format6(_conditionName2), ". Possible values are ").concat(Object.keys(sprinkle2.values[_value2].conditions).map(format6).join(", ")));
                  }
                }
              }
            }
    ...
    if (args_.length > 0) {
      ...
      topics = (indexedInputs == null ? void 0 : indexedInputs.map((param, i) => {
        if (Array.isArray(args_[i]))
          return args_[i].map((_, j) => encode8(param, args_[i][j]));
        return typeof args_[i] !== "undefined" && args_[i] !== null ? encode8(param, args_[i]) : null;
      })) ?? [];
    }
  }
  const selector = (() => {
...
function fromBytes2(bytes, options = {}) {
  ...
  return from9(bytes, { as });
}
function fromHex2(hex2, options = {}) {
  const { as = "Hex" } = options;
  return from9(hex2, { as });
}
...
function fromDerHex(signature) {
  const { r: r2, s } = secp256k1.Signature.fromDER(from2(signature).slice(2));
  return { r: r2, s };
}
...
function fromTuple2(tuple) {
  const [chainId, address, nonce, yParity, r2, s] = tuple;
  const signature = yParity && r2 && s ? fromTuple([yParity, r2, s]) : void 0;
  return from11({
    address,
    chainId: Number(chainId),
    nonce: BigInt(nonce),
...
function hash(authorization) {
  return keccak256(concat2("0x05", fromHex2(toTuple2(authorization))));
}
...
function fromHex5(value, options = {}) {
  return fromBytes5(fromHex(value), options);
}
  ...
  function inner(node) {
    ...
    while (level.length > 1) {
      ...
      for (let i = 0; i < level.length; i += 2)
        level_.push(hash2(concat(level[i], level[i + 1])));
      level = level_;
    }
...
var InvalidVersionedHashSizeError = class extends BaseError {
  constructor({ hash: hash8, size: size4 }) {
...
var InvalidVersionedHashVersionError = class extends BaseError {
  constructor({ hash: hash8, version: version2 }) {
...
function pointG2FromBytes(bytes) {
  ...
  const slc = (b, from26, to3) => bytesToNumberBE(b.slice(from26, to3));
...
function fromHex6(hex2, group) {
  return fromBytes6(toBytes2(hex2), group);
}
...
function fromCreate(options) {
  const from26 = fromHex(from4(options.from));
  let nonce = fromNumber(options.nonce);
  if (nonce[0] === 0)
    nonce = new Uint8Array([]);
  return from4(`0x${keccak256(fromBytes2([from26, nonce], { as: "Hex" })).slice(26)}`);
}
function fromCreate2(options) {
  ...
  return from4(slice2(keccak256(concat(fromHex("0xff"), from26, salt, bytecodeHash), { as: "Hex" }), 12));
}
...
function chain(...args) {
  ...
  const encode8 = args.map((x) => x.encode).reduceRight(wrap, id);
  const decode4 = args.map((x) => x.decode).reduce(wrap, id);
  return { encode: encode8, decode: decode4 };
}
...
function normalize(fn) {
  afn(fn);
  return { encode: (from26) => from26, decode: (to3) => fn(to3) };
}
function convertRadix(data, from26, to3) {
  if (from26 < 2)
...
var radix2carry = (from26, to3) => from26 + (to3 - gcd(from26, to3));
var powers = (() => {
  let res = [];
  for (let i = 0; i < 40; i++)
    res.push(2 ** i);
...
function convertRadix2(data, from26, to3, padding2) {
  aArr(data);
  if (from26 <= 0 || from26 > 32)
    throw new Error(`convertRadix2: wrong from=${from26}`);
  if (to3 <= 0 || to3 > 32)
    throw new Error(`convertRadix2: wrong to=${to3}`);
  if (radix2carry(from26, to3) > 32) {
    throw new Error(`convertRadix2: carry overflow from=${from26} to=${to3} carryBits=${radix2carry(from26, to3)}`);
  }
  let carry = 0;
  let pos = 0;
  ...
  for (const n of data) {
    anumber2(n);
    if (n >= max)
      throw new Error(`convertRadix2: invalid data word=${n} from=${from26}`);
    carry = carry << from26 | n;
    if (pos + from26 > 32)
...
function sign2(options) {
  ...
  const { r: r2, s, recovery } = secp256k1.sign(from(payload), from(privateKey), {
...
function verify2(options) {
  ...
  return secp256k1.verify(signature, from(payload), toBytes3(publicKey), ...hash8 ? [{ prehash: true, lowS: true }] : []);
}
...
function wrapConstructorWithKey(hashCons) {
  const hashC = (msg, key) => hashCons(key, msg.length).update(toBytes10(msg)).digest();
...
function pbkdf2(hash8, password, salt, opts) {
  const { c, dkLen, DK, PRF, PRFSalt } = pbkdf2Init(hash8, password, salt, opts);
  let prfW;
...
async function pbkdf2Async(hash8, password, salt, opts) {
  const { c, dkLen, asyncTick, DK, PRF, PRFSalt } = pbkdf2Init(hash8, password, salt, opts);
  let prfW;
  const arr = new Uint8Array(4);
  const view = createView(arr);
  const u = new Uint8Array(PRF.outputLen);
...
function recoverPublicKey2(options) {
  const { payload, signature } = options;
  const { r: r2, s, yParity } = signature;
  const signature_ = new secp256r1.Signature(BigInt(r2), BigInt(s)).addRecoveryBit(yParity);
...
function sign3(options) {
  const { extraEntropy: extraEntropy2 = extraEntropy, hash: hash8, payload, privateKey } = options;
  const { r: r2, s, recovery } = secp256r1.sign(payload instanceof Uint8Array ? payload : fromHex(payload), privateKey instanceof Uint8Array ? privateKey : fromHex(privateKey), {
    extraEntropy: typeof extraEntropy2 === "boolean" ? extraEntropy2 : from2(extraEntropy2).slice(2),
    lowS: true,
    ...hash8 ? { prehash: true } : {}
  });
...
function verify3(options) {
  ...
  return secp256r1.verify(signature, payload instanceof Uint8Array ? payload : fromHex(payload), toHex2(publicKey).substring(2), ...hash8 ? [{ prehash: true, lowS: true }] : []);
}
...
function deserialize(serialized) {
  ...
  const [nonce, gasPrice, gas, to3, value, data, chainIdOrV_, r2, s] = tuple;
...
function deserialize2(serialized) {
  ...
  const [chainId, nonce, maxPriorityFeePerGas, maxFeePerGas, gas, to3, value, data, accessList, yParity, r2, s] = transactionArray;
...
function deserialize3(serialized) {
  const transactionArray = toHex3(slice2(serialized, 1));
  const [chainId, nonce, gasPrice, gas, to3, value, data, accessList, yParity, r2, s] = transactionArray;
...
function assert6(envelope) {
  ...
  if (blobVersionedHashes) {
    ...
    for (const hash8 of blobVersionedHashes) {
      const size4 = size2(hash8);
      const version2 = toNumber(slice2(hash8, 0, 1));
      if (size4 !== 32)
        throw new InvalidVersionedHashSizeError({ hash: hash8, size: size4 });
...
function deserialize4(serialized) {
  ...
  const [chainId, nonce, maxPriorityFeePerGas, maxFeePerGas, gas, to3, value, data, accessList, maxFeePerBlobGas, blobVersionedHashes, yParity, r2, s] = transactionArray;
...
function deserialize5(serialized) {
  const transactionArray = toHex3(slice2(serialized, 1));
  const [chainId, nonce, maxPriorityFeePerGas, maxFeePerGas, gas, to3, value, data, accessList, authorizationList, yParity, r2, s] = transactionArray;
...
function encodeField(properties) {
  ...
  if (types[type6] !== void 0)
    return [
      { type: "bytes32" },
      keccak256(encodeData({ data: value, primaryType: type6, types }))
    ];
  if (type6 === "bytes") {
...
function getSignPayload9(value) {
  return keccak256(encode6(value));
}
...
function getClientDataJSON(options) {
  ...
  return JSON.stringify({
    type: "webauthn.get",
    challenge: fromHex5(challenge, { url: true, pad: false }),
    origin,
    crossOrigin,
    ...extraClientData
  });
...
function alphabet2(alphabet3) {
  return {
    has: (char) => alphabet3.includes(char),
    decode: (input) => {
...
var info = {
  ...
  alphabet: {
    numeric: alphabet2("0123456789"),
    alphanumerc: alphabet2("0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ $%*+-./:")
  },
...
function RS(eccWords) {
  return {
    encode(from26) {
      ...
      const pol = Array.from(from26);
      pol.push(...d.slice(0, -1).fill(0));
      return Uint8Array.from(GF.remainderPoly(pol, d));
    },
    decode(to3) {
      const res = to3.slice();
      const poly = GF.polynomial(Array.from(to3));
...
function encode7(ver, ecc, data, type6) {
  let encoded = "";
...
function encodeQR(text, output = "raw", opts = {}) {
  ...
  if (ver !== void 0) {
    validateVersion(ver);
    data = encode7(ver, ecc, text, encoding);
  } else {
    for (let i = 1; i <= 40; i++) {
      try {
        data = encode7(i, ecc, text, encoding);
        ver = i;
        break;
      } catch (e) {
        err = e;
...
(function(Cuer2) {
  ...
  function Root(props) {
    const { children, size: size4 = "100%", value, version: version2, ...rest } = props;
    ...
    }, [value, hasArena, props.errorCorrection, version2]);
    const cellSize = 1;
  ...
  return createConnector((config) => ({
    ...
    async getProvider() {
      if (!walletProvider) {
        ...
        const { createCoinbaseWalletSDK } = await import("./dist-BTDA6R4V.js");
    ...
    async getProvider() {
      ...
      if (!walletProvider) {
        const CoinbaseWalletSDK = await (async () => {
          const { default: SDK } = await import("./dist-JAXMNAQO.js");
    ...
    async getProvider() {
      async function initProvider() {
        ...
        const MetaMaskSDK = await (async () => {
          const { default: SDK } = await import("./metamask-sdk-BBBJUET6.js");
          if (typeof SDK !== "function" && typeof SDK.default === "function")
            return SDK.default;
          return SDK;
        })();
  ...
  return createConnector((config) => ({
    ...
    async getProvider() {
      ...
      if (!isIframe)
        return;
      if (!provider_) {
        const { default: SDK } = await import("./esm-2N2WX3DD.js");
        ...
        const SafeAppProvider = await (async () => {
          const Provider = await import("./dist-N637LNJD.js");
    ...
    async getProvider({ chainId } = {}) {
      ...
      async function initProvider() {
        ...
        if (!optionalChains.length)
          return;
        const { EthereumProvider } = await import("./index.es-DIJERJKF.js");
...
var fetchTranslations = async (locale) => {
  switch (locale) {
    ...
    case "en":
    case "en-US":
      return (await import("./en_US-RFN65H63-ZBWZX3LV.js")).default;
    case "es":
    case "es-419":
...
var coinbaseWallet2 = ({ appName, appIcon }) => {
  ...
  return {
    ...
    shortName: "Coinbase",
    rdns: "com.coinbase.wallet",
    iconUrl: async () => (await import("./coinbaseWallet-OKXU3TRC-KU2CC52F.js")).default,
    iconAccent: "#2c5ff6",
